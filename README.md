...
- `wrangler.toml`：Cloudflare 部署配置示例
...
2. 构建设置里将输出目录设置为 `public`；如果使用 `wrangler.toml`，配置已经写好。
...
如果使用 Wrangler 部署，请先把 `wrangler.toml` 里的 `id` 和 `preview_id` 替换成自己的 KV Namespace ID。
